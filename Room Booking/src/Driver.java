...
public class Driver {
	...
	static BookingManager system = new BookingManager(dataFile);
    public static void main(String args[]){
		...
		do{
			...
			switch (i){
				...
				case 'd': addClient();
				break;
				case 'e': system.saveData(dataFile);running = false;
				break;
			}
		}while(running);
    }
    ...
    private static void cancelBooking(){
    	boolean valid;
    	int ref = 0;
    	System.out.println("Enter the booking reference you want to cancel: ");
		do{
			try{
				ref = in.nextInt();
				in.nextLine(); //flushing the input
				valid = true;
			}catch (InputMismatchException e){
				System.out.println("Invalid entry. Please enter a reference number: ");
				valid = false;
    ...
    private static void generateReport(){
		...
		LinkedList<Booking> clientBookings = new LinkedList<Booking>();
		...
		if (!system.clientExists(familyName, givenName)) {
			...
			for (Map.Entry<String, Room> entry : system.getRooms().entrySet()) {
				Room room = entry.getValue();
				for (Booking booking: room.getBookings()
					 ) {
					if(booking.getBooker().getFamilyName().equals(familyName) && booking.getBooker().getGivenName().equals(givenName)){
    ...
    private static void addClient(){
		...
		}while(system.clientExists(familyName, givenName));


		system.addClient(familyName, givenName, email, number);
		System.out.println(system.getClients());
    }
}
